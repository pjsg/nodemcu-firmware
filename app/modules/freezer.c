...
static void *find_data_clean_header(const void *src, size_t len) {
  struct _header {
    CommonHeader;
  } header;

  memset(&header, 0, sizeof(header));
  header.tt = ((struct _header *) src)->tt;
  return find_data(&header, sizeof(header), ((struct _header *) src) + 1, len - sizeof(header));
}
...
static int do_freeze_proto(lua_State *L, Proto **fp) {
  int i;

  PENDING_LIST protos;
  memset(&protos, 0, sizeof(protos));

  pending_push(&protos, fp);

  size_t freed = 0;

  while (fp = (Proto **) pending_get(&protos)) {
    Proto *f = *fp;
    ...
    if (opts & OPT_PROTO) {
      if (!is_flash(f)) {
	if (is_flash(f->source) && is_flash(f->code) && 
	  (!f->sizeupvalues || is_flash(f->upvalues)) && 
	  (!f->sizelocvars || is_flash(f->locvars)) && 
	  (!f->sizek || is_flash(f->k)) && 
	  is_flash(f->packedlineinfo) &&
	  !f->sizep) {
	  Proto *nf = (Proto *) find_data_clean_header(f, sizeof(*f));
	  if (nf) {
	    *fp = nf;
	  }
	}
...
static int do_freeze_closure(lua_State *L, Closure *cl) {
  ...
  while (cl = (Closure *) pending_get(&closures)) {
    ...
    result += do_freeze_proto(L, &(cl->l.p));
