...
static int lswitec_setup( lua_State* L )
{
  ...
  // Verify pins differences. either ABD all different and B = C or ABCD all different
  int pincount = 0;
  int pinmask = 0;
  for (i = 0; i < 4; i++) {
    int pinbit = 1 << pin[i];
  ...
  if (pincount < 3 || (pincount == 3 && pin[1] != pin[2])) {
    return luaL_error( L, "Pin combination not appropriate." );
  }
