...
static u8_t spiffs_fds[sizeof(spiffs_fd) * CONFIG_NODE_SPIFFS_MAX_OPEN_FILES];
#if SPIFFS_CACHE
...
static int32_t myspiffs_vfs_readdir( const struct vfs_dir *dd, struct vfs_stat *buf ) {
  ...
  if (SPIFFS_readdir( d, &dirent )) {
    ...
    strncpy( buf->name, (char *)dirent.name, CONFIG_NODE_FS_OBJ_NAME_LEN+1 );
    buf->name[CONFIG_NODE_FS_OBJ_NAME_LEN] = '\0';
...
static int32_t myspiffs_vfs_stat( const char *name, struct vfs_stat *buf ) {
  ...
  if (0 <= SPIFFS_stat( &fs, name, &stat )) {
    ...
    // fill in supported stat entries
    strncpy( buf->name, (char *)stat.name, CONFIG_NODE_FS_OBJ_NAME_LEN+1 );
    buf->name[CONFIG_NODE_FS_OBJ_NAME_LEN] = '\0';
    buf->size = stat.size;
    return VFS_RES_OK;
  } else {
