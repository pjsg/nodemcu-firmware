...
uint32_t platform_uart_setup( unsigned id, uint32_t baud, int databits, int parity, int stopbits, uart_pins_t* pins )
{
  ...
  else
  {
    ...
    uart_set_line_inverse(id, (pins->tx_inverse? UART_SIGNAL_TXD_INV : UART_SIGNAL_INV_DISABLE)
                                | (pins->rx_inverse? UART_SIGNAL_RXD_INV : UART_SIGNAL_INV_DISABLE)
                                | (pins->rts_inverse? UART_SIGNAL_RTS_INV : UART_SIGNAL_INV_DISABLE)
                                | (pins->cts_inverse? UART_SIGNAL_CTS_INV : UART_SIGNAL_INV_DISABLE)
                        );
